...
describes what users get. Pick a format with `-o`, or let cn360 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cn360 <command> -o table   # aligned columns for reading
cn360 <command> -o jsonl   # one JSON object per line, for piping
cn360 <command> -o json    # a single JSON array
cn360 <command> -o csv     # spreadsheet friendly
cn360 <command> -o tsv     # tab-separated
cn360 <command> -o url     # just the URL column
cn360 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cn360 <command> --fields id,title,url
```
...
```bash
cn360 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cn360 <command>            # a table, because this is a terminal
cn360 <command> | wc -l    # JSONL, because this is a pipe
```
